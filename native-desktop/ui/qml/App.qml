...
Window {
    ...
    property bool snapped: false; property int snapDir: 0
    property int savedX: 0; property int savedY: 0

    Timer { id: snapCheckTimer; interval: 400; repeat: true; running: true
    ...
    Image {
        ...
        MouseArea {
            anchors.fill: parent
            property point lp: Qt.point(0,0); property bool d: false
            ...
            onReleased: m => {
                ...
                else { petImage.stateName = states[Math.floor(Math.random()*states.length)] }
                d=false
            }
            onDoubleClicked: {
                var msgs = ["你好！","今天过得怎么样？","讲个笑话吧","我有点难过","晚安"]
                writeInput(msgs[Math.floor(Math.random()*msgs.length)])
            }
        }
    }
    ...
    Rectangle {
        ...
        Text { id: label; anchors.centerIn: parent; color: "white"; font.pixelSize: 12
            text: {
                switch(petImage.stateName){case"idle":return"待机";case"greeting":return"问候"
                case"thinking":return"思考";case"happy":return"开心";case"sad":return"难过"
                case"surprised":return"惊讶";case"listening":return"倾听";case"sleeping":return"睡觉"
    ...
    Timer { interval: 10000; running: root.chatText.length>0; repeat: false
        onTriggered: root.chatText="" }

    function writeInput(msg) {
        var xhr = new XMLHttpRequest()
        xhr.open("PUT", "file:///C:/tmp/deskpet-bridge/input.json", false)
        xhr.send('{"action":"chat","message":"'+msg+'","zone":"","switch_to":""}')
    }
